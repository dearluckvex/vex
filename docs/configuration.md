...
> 👉 初次使用请先阅读 [快速开始](./getting-started.md)。


## 内部配置格式

Vex 使用统一的内部 YAML 配置格式，所有导入的配置最终都会转换为此格式。
...
Clash 是最广泛使用的代理配置格式。Vex 解析其 `proxies` 字段。
...
**字段映射**: Clash `type` → Vex `protocol.type`
| Clash type | Vex protocol |
|-----------|---------------|
| `ss` | `shadowsocks` |
| `vmess` | `vmess` |
| `vless` | `vless` |
...
| 平台 | 路径 |
|------|------|
| Windows | `%APPDATA%\vex\config.yaml` |
| macOS | `~/Library/Application Support/vex/config.yaml` |
| Linux | `~/.config/vex/config.yaml` |
