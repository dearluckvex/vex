...
├── crates/
│   ├── xtune-core/         # 核心库（协议、配置解析、代理服务）
│   ├── xtune-gui/          # GUI 客户端 (gpui)
│   └── xtune-cli/          # CLI 客户端
├── docs/                   # 文档
└── target/                 # 编译输出（自动生成）
```
...
```bash
# 仅核心库
cargo build -p xtune-core

# 仅 CLI
cargo build -p xtune-cli

# 仅 GUI
cargo build -p xtune-gui
```
...
```bash
cargo run -p xtune-cli -- config.yaml
```
...
cargo test -p xtune-core

# GUI 测试
cargo test -p xtune-gui

# CLI 测试
cargo test -p xtune-cli
```
...
### Q: 重启后节点列表丢失
GUI 会自动将节点保存到 `~/.config/xtune/gui-state.yaml`（Linux/macOS）或 `%APPDATA%\xtune\gui-state.yaml`（Windows）。如果路径不存在程序会自动创建。
