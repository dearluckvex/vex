...
**位置**：`crates/vex-core/src/router/engine.rs` 第 158-162 行
...
**位置**：`crates/vex-core/src/dns/mod.rs` 第 204-217 行
...
**位置**：`crates/vex-core/src/proxy/relay.rs`
...
**位置**：`crates/vex-core/src/proxy/speedtest.rs` 第 20-66 行
...
4. **线程模型与 UI 框架一致**：GPUI（类似 macOS AppKit / WinUI）要求 UI 操作在主线程，vex 正确遵守
...
- vex 实现：单个 future，每次 poll 驱动两个方向，减少调度开销
...
**位置**：`crates/vex-core/src/config/subscription.rs`
